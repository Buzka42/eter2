...
| `seriesLabel()` | stepsyour step countactiveKcalhow much you movedsleephow long you sleptdeepdeep sleepremREM sleepawaketime awake in the nightrestingHryour resting heart ratehrvyour heart-rate variabilityintakewhat you atesessionstraining sessionsmoodyour moodstressyour stressrecoveryhow recovered you feltmeditationtime spent in meditationweightyour weight | stepstwoja liczba krokówactiveKcalile się ruszałaś lub ruszałeśsleepjak długo spałaś lub spałeśdeepsen głębokiremsen REMawakeczas czuwania w nocyrestingHrtwój puls spoczynkowyhrvtwoja zmienność rytmu sercaintaketo, co jadłaś lub jadłeśsessionssesje treningowemoodtwój nastrójstresstwój stresrecoveryjak zregenerowana lub zregenerowany się czułaś lub czułeśmeditationczas spędzony na medytacjiweighttwoja waga |
| `retrospectiveHeadline()` | Your seven-day viewYour partial seven-day view | Twój skrót siedmiu dniTwój częściowy skrót siedmiu dni |
| `retrospectiveSleep()` | Sleep was available for $nights of 7 nights, averaging $averageHours hours. | Sen był dostępny dla $nights z 7 nocy, średnio $averageHours godz. |
| `retrospectiveJournal()` | You made $entries journal ${entries == 1 ? : } during this window. | W tym okresie zrobiono $entries ${_wpisy(entries)} w dzienniku. |
| `retrospectiveLifestyle()` | $signals self-reported ${signals == 1 ? : } recorded across ${kinds.map(lifestyleKindName).join()}. | Zapisano $signals ${_sygnaly(signals)} zgłoszone samodzielnie, w kategoriach: ${kinds.map(lifestyleKindName).join()}. |
