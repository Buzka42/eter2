...
| Dashboard | `app/lib/features/dashboard/dashboard_page.dart`, `body_section.dart` | Synthesis owns the resting view above one `LOOK DEEPER` threshold. Its in-place chooser opens expanded Guidance or Body, never simultaneous rows. The only disclosure glyph is the code-native bead-and-thread `EterDisclosureMark`; no Material icon remains on production surfaces. **The Dashboard reads; the Journal writes** (28 July 2026): every capture control has left this surface — no add activity, add meal, record strength or record weight — and the expanded Body is now a reading in one order. Guidance composes itself on the day's first look, once per local day, when a transport exists; the explicit `COMPOSE NOW` is a quiet retry that disappears while composing. Expanded Guidance offers `REFRESH`, reuses unchanged context without another provider call, preserves existing content on failure, and announces status accessibly. Production `sentences + primaryAction` responses and legacy fixture caches both render as prose. Guidance renders Health/Mind/Spirit prose and marginal evidence receipts. Body begins with a conclusion, states missing signals, then recovery prose, the redrawn `EngravedBalance`, engraved RHR/HRV/weight trends, a sleep rail whose legend is struck in each stage's own weight, the sleep history, activity by time, and food notes. The one interaction that remains is correcting a derived food estimate, which is review rather than input; estimates are visibly not counted until corrected or confirmed. The manual write services survive untouched and tested — only the surface moved — and `ROADMAP.md` §1a tracks the half this leaves unfinished: weight, activity and strength have no route in until journal classification grows shapes for them. Every instrument has a complete semantic summary. |
| Journal | `app/lib/features/journal/journal_page.dart` | One screen that does not scroll (28 July 2026). The date and `HISTORY` lead; the day's story takes whatever height is left and fits itself to it by binary search over point size, never truncating; the ruled writing field and one `DICTATE` action — the code-native `EterMicMark` plus the word — sit on the floor of the page. Everything already written lives in the `HISTORY` sheet with day navigation, which also saves a draft before it opens. Invisible 900 ms autosave (`speech_to_text` for dictation). No checkmarks, Done/Save action, toggle or composer chrome. The bead-and-thread page-turn marks moved into that sheet: turning pages is reading history. Future days are disabled and historical pages are read-only. Entries read as continuous timed prose and arrive via the shared reveal. The page leads with the day's story — a few sentences Aether writes from everything recorded that day, labelled `THE DAY SO FAR`, refreshed on open and after each save, and free when the day's prose has not changed. The marginal mood/stress/recovery check-ins were removed on 28 July 2026: interpretation derives mood, energy and sleep from the prose itself, so a separate self-report asked twice for the same thing. Each saved passage has quiet `KEEP LOCAL` / `ALLOW AETHER` and explicit `INTERPRET` actions. Interpretation requires current AI consent, supports one bounded clarification, announces progress/failure accessibly, and offers `UNDO INTERPRETATION`, which removes the derived rows without deleting the prose. Excluded prose remains omitted from guidance context. |
